...
describe File.basename(__FILE__) do
	before(:all) do
		...
		@job_data[:callbacks] << {
			:type => MovieMasher::Transfer::TypeFile,
			:trigger => MovieMasher::Callback::TriggerError,           # request only if error encountered
			:name => 'callback.txt',
			:directory => @job_data[:destination][:directory],
			:data => {                          # body of request, JSON formatted
				:result => '{job.error}',        # String - Job#error
			}
		}
