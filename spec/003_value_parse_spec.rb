...
describe File.basename(__FILE__) do
	...
	context "__filter_parse_scope_value" do
		it "returns value when sent just identifier: identifier" do
			value = "identifier"
			scope = Hash.new
			expect(filter.send :__filter_parse_scope_value, scope, value).to eq value
		end
		it "returns expresions when sent simple expression: 2 + a * m - j" do
			value = "2 + a * m - j"
			scope = Hash.new
			expect(filter.send :__filter_parse_scope_value, scope, value).to eq '2+a*m-j'
		end
		it "returns proper nested array for nested non calls: (in_h-mm_max(mm_width, mm_height))-((in_h-mm_max(mm_width, mm_height))*mm_t)" do
			...
			expect(filter.send :__filter_parse_scope_value, scope, value).to eq result
		end
